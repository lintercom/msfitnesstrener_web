        ...
        <div className="fixed bottom-8 right-8 md:bottom-10 md:right-10 z-[60] flex flex-col items-center">
            ...
            {!isHomePage && (
                ...
                >
                    ...
                    <div className={`absolute inset-0 neon-gradient transition-opacity duration-500 rounded-full ${
                        isOverFooter ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                    }`}></div>
            ...
            <button
                type="button"
                onClick={scrollToTop}
                className={`w-16 h-16 md:w-20 md:h-20 rounded-full flex items-center justify-center transition-all duration-500 ease-in-out transform-gpu ${
                    isScrollVisible ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-50 translate-y-4 pointer-events-none'
