                ...
                {services.map((service, index) => {
                    ...
                    return (
                        <section 
                            key={service.id} 
                            id={service.id}
                            className={`group w-[94%] lg:w-[92%] max-w-[1300px] relative transition-shadow duration-500 scroll-mt-24 ${shadowClass} ${
                                isEven ? 'mr-auto rounded-r-[3rem]' : 'ml-auto rounded-l-[3rem]'
                            }`}
                        >
                            <div 
                                className={`flex flex-col ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'} min-h-0 md:min-h-[400px] lg:min-h-[480px] bg-surface-light border-none overflow-hidden ${
                                    isEven ? 'rounded-r-[3rem]' : 'rounded-l-[3rem]'
                                }`}
                                style={{ 
