                        ...
                        {status === 'success' ? (
                            ...
                            <div className="relative">
                                ...
                                <div className={`transition-all duration-700 ease-in-out ${activeTab === 'existing' ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-10 scale-95 pointer-events-none absolute inset-0'}`}>
                                    <div className="bg-surface-light/80 backdrop-blur-sm border border-surface-dark/5 rounded-[3rem] p-12 md:p-20 shadow-premium relative overflow-hidden group flex flex-col items-center text-center">
                                        <div className="w-24 h-24 bg-surface-dark/5 rounded-[2rem] flex items-center justify-center text-surface-dark/20 mb-10 group-hover:neon-gradient group-hover:text-white transition-all duration-500"><svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg></div>
                                        <h2 className="text-4xl md:text-5xl font-black text-surface-dark uppercase tracking-tighter leading-none mb-6">Klientský <span className="ms-gradient-text">portálu</span></h2>
                                        <p className="text-surface-dark/70 font-medium text-lg max-w-lg mb-12">Na vývoji členské sekce usilovně pracujeme. Brzy zde najdeš svůj tréninkový deník a progresové grafy.</p>
                                        <div className="px-8 py-3 bg-surface-dark text-white rounded-full text-[10px] font-black uppercase tracking-[0.4em]">UNDER CONSTRUCTION</div>
                                    </div>
                                </div>
                            </div>
                        )}
